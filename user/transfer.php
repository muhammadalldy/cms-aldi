...
$sql= "
SELECT 'f' AS ctg, s.matrix_no, s.name AS student_name, sf.kode_matkul_diakui AS subject_code, sbj.subject_eng, sbj.sub_desc AS subject_id, 
sf.id_periode_masuk AS semester_id, sf.nilai_huruf_diakui AS grade, 
sf.sks_mata_kuliah_diakui AS cre_hour, gt.grade_index AS grade_index,
(gt.grade_index*sf.sks_mata_kuliah_diakui) AS weight,
sbj.sub_desc,
sf.id_periode_masuk,
ss.semester_jgu
FROM student s
LEFT JOIN transfer_score_feeder sf ON sf.nim = s.rel_mat_no
LEFT JOIN score_feeder aa ON (aa.kode_mata_kuliah = sf.kode_matkul_diakui AND aa.nim = sf.nim)
LEFT JOIN `subject` sbj ON sbj.subject_code = sf.kode_matkul_diakui
LEFT JOIN grade_type gt ON gt.grade = sf.nilai_huruf_diakui
LEFT JOIN semester_feeder ss ON ss.semester = sf.id_periode_masuk
WHERE s.matrix_no='".addslashes($_GET['id'])."'
AND aa.nim IS NULL
ORDER BY sf.kode_matkul_diakui, sf.id_periode_masuk
";
...
$mpdf->WriteHTML('<table style="width: 60%; font-size: 12px">
                    ...
                    <td style="width:5px;font-size: 9px; font-family: Arial"><b>:</b></td>
                    <td style="width:40%;font-size: 9px; font-family: Arial"><b>Transfer</b></td>
                    </tr> 
                    </table>');
...
} elseif($totalsquare == 2){
	$div2 = 40;
	$sisa = $div3 - $all;
	include('transfer2.php');
} elseif($totalsquare == 3){
	$div3 = 60;
	$sisa = $div3 - $all;
	include('transfer3.php');
} elseif($totalsquare == 4){
	$div4 = 80;
	$sisa = $div4 - $all;

	include('transfer4.php');
